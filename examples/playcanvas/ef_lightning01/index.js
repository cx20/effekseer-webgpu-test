...
function main() {
  ...
  cameraEntity.addComponent("camera", {
    ...
    fov: 45, nearClip: 1, farClip: 1000,
  });
  app.root.addChild(cameraEntity);

  const lightEntity = new pc.Entity("light");
  lightEntity.addComponent("light", { type: "directional", color: new pc.Color(1, 1, 1), intensity: 3 });
  ...
  let theta = Math.PI / 4, phi = Math.acos(15 / 26), radius = 26;

  function getCameraPos() {
    return { x: radius * Math.sin(phi) * Math.sin(theta), y: radius * Math.cos(phi), z: radius * Math.sin(phi) * Math.cos(theta) };
  }
  ...
  effekseer.initRuntime("../../effekseer/effekseer-webgl.wasm", () => {
    ...
    const effect = context.loadEffect(EFFECT_PATH, 1.0, () => {
      setStatus("Ready");

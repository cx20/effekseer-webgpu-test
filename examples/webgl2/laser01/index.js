...
// Column-major WebGL matrix utilities
...
function matLookAt(ex, ey, ez, cx, cy, cz) {
  ...
  fx /= d; fy /= d; fz /= d;
  let rx = -fz, ry = 0, rz = fx; // right = forward × (0,1,0)
...
out vec4 c;
void main() {
  float d = max(dot(normalize(vNorm), normalize(vec3(1.0, 2.0, 1.0))), 0.0) * 0.7 + 0.3;
  c = vec4(0.27 * d, 0.53 * d, 1.0 * d, 1.0);
}`;
...
function main() {
  ...
  effekseer.initRuntime("../../effekseer/effekseer-webgl.wasm", () => {
    ...
    function render() {
      ...
      const proj = matPerspective(45, w / h, 1, 1000);
      const view = matLookAt(...EYE, 0, 0, 0);
      const model = matRotYTranslate(t, 0, 1, 0);

      gl.clearColor(0.082, 0.098, 0.122, 1);
      ...
      const aCubePos = gl.getAttribLocation(cubeProg, "aPos");
      const aCubeNorm = gl.getAttribLocation(cubeProg, "aNorm");
      gl.enableVertexAttribArray(aCubePos);
      gl.enableVertexAttribArray(aCubeNorm);
      gl.vertexAttribPointer(aCubePos, 3, gl.FLOAT, false, 24, 0);
      gl.vertexAttribPointer(aCubeNorm, 3, gl.FLOAT, false, 24, 12);

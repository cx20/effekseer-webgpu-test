...
function resizeEffekseerCanvas(context) {
  ...
  if (effekseerCanvas.width !== w || effekseerCanvas.height !== h) {
    effekseerCanvas.width = w; effekseerCanvas.height = h;
    context?.configureSurface({ width: w, height: h });
  }
}
...
async function main() {
  ...
  let depthTexture = device.createTexture({
    size: [sceneCanvas.width, sceneCanvas.height],
    format: "depth24plus",
...
@vertex fn vsGrid(@location(0) p: vec3<f32>) -> @builtin(position) vec4<f32> {
  return u.projection * u.view * vec4<f32>(p, 1.0);
}
...
@vertex fn vsCube(@location(0) p: vec3<f32>, @location(1) n: vec3<f32>) -> VO {
  var o: VO;
  o.p = u.projection * u.view * u.model * vec4<f32>(p, 1.0);
  o.n = (u.model * vec4<f32>(n, 0.0)).xyz;
  return o;
}
@fragment fn fsCube(@location(0) n: vec3<f32>) -> @location(0) vec4<f32> {
  let d = max(dot(normalize(n), normalize(vec3<f32>(1.0, 2.0, 1.0))), 0.0) * 0.7 + 0.3;
  return vec4<f32>(0.27 * d, 0.53 * d, 1.0 * d, 1.0);
}`
  });
  ...
  const gridPipeline = device.createRenderPipeline({
    ...
    vertex: { module: shaderModule, entryPoint: "vsGrid", buffers: [{ arrayStride: 12, attributes: [{ shaderLocation: 0, offset: 0, format: "float32x3" }] }] },
    fragment: { module: shaderModule, entryPoint: "fsGrid", targets: [{ format }] },
  ...
  const cubePipeline = device.createRenderPipeline({
    layout: pipelineLayout,
    vertex: { module: shaderModule, entryPoint: "vsCube", buffers: [{ arrayStride: 24, attributes: [{ shaderLocation: 0, offset: 0, format: "float32x3" }, { shaderLocation: 1, offset: 12, format: "float32x3" }] }] },
    fragment: { module: shaderModule, entryPoint: "fsCube", targets: [{ format }] },
  ...
  function render() {
    ...
    uniformData.set(proj, 0); uniformData.set(view, 16); uniformData.set(model, 32);
    device.queue.writeBuffer(uniformBuffer, 0, uniformData);

    const enc = device.createCommandEncoder();
    const pass = enc.beginRenderPass({
      colorAttachments: [{ view: sceneCtx.getCurrentTexture().createView(), clearValue: { r: 0.082, g: 0.098, b: 0.122, a: 1 }, loadOp: "clear", storeOp: "store" }],
  ...
  window.addEventListener("resize", () => {
    ...
    sceneCanvas.width = Math.max(1, Math.floor(window.innerWidth * pr2));
    sceneCanvas.height = Math.max(1, Math.floor(window.innerHeight * pr2));
    sceneCtx.configure({ device, format, alphaMode: "opaque" });
    depthTexture.destroy();
    depthTexture = device.createTexture({ size: [sceneCanvas.width, sceneCanvas.height], format: "depth24plus", usage: GPUTextureUsage.RENDER_ATTACHMENT });

...
@fragment fn fsCube(@location(0)n:vec3<f32>)->@location(0)vec4<f32>{let d=max(dot(normalize(n),normalize(vec3<f32>(1.0,2.0,1.0))),0.0)*0.7+0.3;return vec4<f32>(0.27*d,0.53*d,1.0*d,1.0);}`});
  ...
  const gridPipeline = device.createRenderPipeline({layout:pl, vertex:{module:shaderModule,entryPoint:"vsGrid",buffers:[{arrayStride:12,attributes:[{shaderLocation:0,offset:0,format:"float32x3"}]}]}, fragment:{module:shaderModule,entryPoint:"fsGrid",targets:[{format}]}, primitive:{topology:"line-list"}, depthStencil:{format:"depth24plus",depthWriteEnabled:true,depthCompare:"less"}});
  ...
  const loadedEffects = {};
  await Promise.all(EFFECT_DEFS.map(async({name,path})=>{loadedEffects[name]=await context.loadEffect(path);}));
  setStatus("Ready");

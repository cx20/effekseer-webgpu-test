...
const EFFECT_PATH = '../../effekseer/Resources/EffectMaterials/ef_lightning01.efkefc';
...
function matLookAt(ex, ey, ez, cx, cy, cz) {
  ...
  let d = Math.sqrt(fx*fx + fy*fy + fz*fz); fx/=d; fy/=d; fz/=d;
...
const load = async function () {
  ...
  await new Promise((resolve, reject) => {
    effekseer.initRuntime('../../effekseer/effekseer-webgl.wasm', () => {
      ...
      const effect = effekseerContext.loadEffect(EFFECT_PATH, 1.0, () => {
        setStatus('Ready');
        ...
        resolve();
      }, (message, url) => { setStatus(`Failed to load effect: ${message} (${url})`, true); reject(new Error(message)); });
    }, () => { setStatus('Failed to initialize Effekseer runtime.', true); reject(new Error('init failed')); });
  });
